...
contract CircleRegistry is AccessControl {
    ...
    // Events
    event CircleCreated(bytes32 indexed circleId, string name, uint256 proposalScope, uint256 requiredStake);
    event CircleUpdated(bytes32 indexed circleId, uint256 proposalScope, uint256 requiredStake);
    ...
    function createCircle(
        ...
        circleId = keccak256(abi.encodePacked(name, block.timestamp, msg.sender));
     ...
     */
    function deactivateCircle(bytes32 circleId) external onlyRole(CIRCLE_ADMIN_ROLE) {
        Circle storage circle = circles[circleId];
        if (circle.createdAt == 0) revert CircleNotFound(circleId);
     ...
     */
    function addMember(bytes32 circleId, address member) external onlyRole(CIRCLE_ADMIN_ROLE) {
        Circle storage circle = circles[circleId];
        if (circle.createdAt == 0) revert CircleNotFound(circleId);
        if (!circle.active) revert CircleNotActive(circleId);
     ...
     */
    function removeMember(bytes32 circleId, address member) external onlyRole(CIRCLE_ADMIN_ROLE) {
     ...
     */
    function getCirclesForMember(address member) external view returns (bytes32[] memory) {
